...
function sourceText(sources: Source[]) {
  return sources.map((s, i) => `SOURCE ${i + 1} — ${String(s.title || 'general').toUpperCase()}\n${s.body}${s.source_url ? `\nURL: ${s.source_url}` : ''}`).join('\n\n');
}
...
async function generate(client: OpenAI, platform: 'threads'|'linkedin'|'substack', sources: Source[]) {
  ...
  const instructions: Record<string, string> = {
    threads: `Create exactly 3 Threads posts. Threads can draw from BOOKS, BLACKVANE, and 2081 material. If both BOOKS material and BLACKVANE/2081 material are supplied, at least one post must come from the BOOKS side and at least one must come from the BLACKVANE/2081 side. Keep brand identities distinct; do not mash unrelated sources together. Make all three posts genuinely different in angle and phrasing. Keep them concise, conversational, sharp, and worth replying to. They can be literary, cultural, business-minded, mission-driven, or provocative depending on the source. Do not force brand mentions. Return ONLY a valid JSON array of 3 strings.`,
    linkedin: `Create exactly 1 LinkedIn post using ONLY the supplied 2081 and/or BLACKVANE material. Each source is labeled with its lane. Keep 2081 and Blackvane distinct. Never attribute a Blackvane idea, service, claim, or fact to 2081, and never attribute a 2081 mission, impact claim, donation figure, or veteran-service fact to Blackvane. Never use BOOKS material here. For BLACKVANE, write substantive observations about organizational diagnostics, revenue, retention, execution, leadership, or the actual business issue in the source. For 2081, write about the veteran mission, impact, service, fundraising, community, or the actual nonprofit issue in the source. If both brands are present, choose the strongest single argument rather than blending them. Avoid corporate inspiration and generic leadership language. Return ONLY a valid JSON array containing 1 string.`,
    substack: `Create exactly 3 Substack Notes using ONLY the supplied BOOKS material. These should support the books and the writing without turning every Note into an advertisement. Use themes, lines, ideas, tensions, excerpts, behind-the-work observations, questions, or provocations supported by the source. Mention a book title when the source gives you one and it helps. Writerly, compact, human, and worth responding to. Never use BLACKVANE or 2081 material here. Return ONLY a valid JSON array of 3 strings.`,
  };
